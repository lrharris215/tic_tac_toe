describe "AdvancedComputerReceiver" do
  ...
  let(:validator) { MoveValidator.new(board, TestConverter.new) }
  ...
  context "CalculateBestMove" do
    it "will win the game if a winning move is available" do
      ...
      expect(advanced.get_player_input(validator, display, computer, human)).to eq(3)
    end

    it "will block a human player from winning" do
      board.place_mark(human.mark, 1)
      board.place_mark(human.mark, 4)
      expect(advanced.get_player_input(validator, display, computer, human)).to eq(7)
    end
    ...
    it "will create a fork" do
      ...
      expect(advanced.get_player_input(validator, display, computer, human)).to eq(1)
    end
    ...
    it "will pick the center square if it's available" do
      expect(advanced.get_player_input(validator, display, computer, human)).to eq(5)
    end

    it "will pick the opposite corner from the human player" do
      board.place_mark(human.mark, 1)
      allow(advanced).to receive(:center).and_return(false)
      expect(advanced.get_player_input(validator, display, computer, human)).to eq(9)
    end

    it "will pick a random corner" do
      allow(advanced).to receive(:center).and_return(false)
      expect([1, 3, 7, 9]).to include(advanced.get_player_input(validator, display, computer, human))
    end
